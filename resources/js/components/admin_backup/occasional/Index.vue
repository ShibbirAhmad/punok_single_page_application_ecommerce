          ...
          <div class="col-lg-8 col-lg-offset-2">
            <div class="box box-primary">
              <div class="box-header text-center with-border">
                <h3 class="box-title">Set Occasional Campaign</h3>
              </div>
              <div class="box-body">
                <div class="alert-danger alert" v-if="error">{{ error }}</div>

                <form
                  @submit.prevent="updateOccasionalCampaign"
                  ...
                  <div class="form-group">
                    <div class="row">
                      <div class="col-md-6">
                        ...
                        />
                        <has-error :form="form" field="product_code_one"> </has-error>
                      </div>
                      <div class="col-md-6">
                        ...
                        />
                         <has-error :form="form" field="product_code_two"> </has-error>
                      </div>
                    </div>
                  </div>
                  <div class="form-group">
                    <label for="campaign_background">Upload campaign_background</label>
                    <img
                      @click="clickImage"
...
export default {
  ...
  methods: {
    getOccasionCampaign() {
      axios.get("/api/get/occasion/campaign")
      .then((resp) => {
        console.log(resp);
        if (resp.data.status == "OK") {
          ...
          if(this.form.campaign_background){
              this.preview_campaign_background=this.base_url+this.form.campaign_background;
           }
        }
      });
    },

    updateOccasionalCampaign() {
      this.form
        .post("/api/edit/occasion/campaign/" + this.form.id, {
          transformRequest: [
